...
'''
from src.classifier import DecisionTreeClassifier, NearestNeighborClassifier, PPRClassifier
import os
import torch
import torchvision
...
class FeatureExtractor:
  def __init__(self):
    ...
    self.handle = self.model.get_submodule('avgpool').register_forward_hook(self.hook)
...
if os.path.exists('artifacts/resnet50_avgpool_features.pkl'):
  ...
  for idx, (image, label) in tqdm(enumerate(DATASET), desc='Extracting Features', total=len(DATASET), leave=False):
      if idx % 2: continue  # Skipping odd images
...
if os.path.exists(ODD_IMG_FEAT_PATH):
    ...
    odd_image_features = []
    trues = []
    for idx, (image, label) in tqdm(enumerate(DATASET), desc='Calculating Odd Image Features', total=len(DATASET), leave=False):
        if idx % 2 == 0: continue  # Skipping even images
...
def main(classifier):
    ...
    for label in tqdm(labels.unique(), desc='Calculating Per-Label Metrics', leave=False):
