    ...
    def hash_data(self, data_vector, layer):
        ...
        for i in range(self.num_random_vectors):
            hash_value = self.hash_function(
                data_vector,
                self.projection_vectors[layer, i, :],
                self.width_vectors[layer, i],
            )
...
def main():
    ...
    if True:
        # Initialize random projection vectors for each layer
        projection_vectors = (
            2 * np.random.rand(num_layers, num_random_vectors, input_dim) - 1
        )
        projection_vectors /= np.linalg.norm(projection_vectors, axis=-1, keepdims=True)
        random_width_vector = (
            np.random.rand(num_layers, num_random_vectors) * 0.1
            + 0.001 * num_layers * num_random_vectors
        )
