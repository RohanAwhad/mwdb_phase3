...
class SVD:
  ...
  def transform(self, feats: torch.Tensor):
    if feats.ndim == 1: feats = feats.unsqueeze(0)
...
if os.path.exists(ODD_IMG_LATENT_FEAT_PATH):
  ...
  for idx, (image, label) in tqdm(enumerate(DATASET), desc='Calculating Odd Image Latent Features', total=len(DATASET), leave=False):
...
for latent_feature in tqdm(odd_image_latent_features, desc='Predicting Labels', leave=False):
